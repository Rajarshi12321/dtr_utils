...
class GenerateBeamDecoderOnlyOutput(ModelOutput):
    ...
    hidden_states: Optional[Tuple[Tuple[torch.FloatTensor]]] = None
    past_key_values: Optional[Tuple[Tuple[Tuple[torch.FloatTensor]]]] = None
    n_indices: Optional[torch.LongTensor] = None
    step_beam_scores: Optional[torch.FloatTensor] = None
    step_beam_tokens: Optional[torch.LongTensor] = None
    step_parent_indices: Optional[torch.LongTensor] = None
    step_extra_tokens: Optional[torch.LongTensor] = None
    step_extra_scores: Optional[torch.FloatTensor] = None
    step_extra_ngram_tokens: Optional[torch.LongTensor] = None
    step_extra_ngram_scores: Optional[torch.FloatTensor] = None
...
class GenerationMixin:
    ...
    def get_ngram_tokens(self,tokenizer,input_tokens,two_gram_probs, three_gram_probs,  four_gram_probs, top_k=4):
        # print("ngram_inputs",input_tokens)
        __token_pob__ = {}
        # print(probs,__token_pob__)
        # print("two_gram_probs",two_gram_probs)
        # print("three_gram_probs",three_gram_probs)
        # print("four_gram_probs",four_gram_probs)
        num = 0
    ...
    def _beam_search(
        ...
        **model_kwargs,
        ...
        depth=0
        pbar = tqdm(total=beam_scorer._beam_hyps[0].max_length,desc="depth")
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            # extra_token_scores=next_token_scores.clone()
            ...
            # print(beam_scores)
            probs=torch.exp(beam_scores)
            max_prob=torch.max(probs)
            # inputs=tokenizer.convert_ids_to_tokens(input_ids,skip_special_tokens=True)
            # print("ngram_input:",inputs)
            ...
            if modified:
                ...
                if max_prob<threshold:
                    ...
                    for i in range(input_ids.shape[0]):
                        inputs=tokenizer.convert_ids_to_tokens(input_ids[i],skip_special_tokens=True)
                    ...
                    # print("all ngram:",all_ngram_tokens)
                    # print("all_indices:",all_ngram_indices)
                    # print("cur_n_grams:",cur_ngrams)
                    ...
                    for i in range(0,max_ngram_token):
                        ...
                        if n_gram_token in beam_next_tokens:
                            continue
                        replace=True
                        beam_next_tokens[replace_idx]=n_gram_token
                    ...
                    if replace:
                        ...
                        beam_idx=beam_idx[sorted_indices]
                        curr_n_indices=curr_n_indices[sorted_indices]
                
                extra_tokens=[[] for i in range(num_beams)]
                extra_token_scores=[[] for i in range(num_beams)]
                # print("next_indices",next_indices)
                for i in range(next_tokens.shape[1]):
                    extra_tokens[next_indices[0,i]].append(next_tokens[0,i])
                    extra_token_scores[next_indices[0,i]].append(next_token_scores[0,i].item())
            ...
            # print("beam_next_inputs",tokenizer.batch_decode(input_ids))
            # This is needed to properly delete outputs.logits which may be very large for first iteration
            # Otherwise a reference to outputs is kept which keeps the logits alive in the next iteration
            ...
            if return_dict_in_generate:
                ...
                # print(beam_indices)
                if modified:
                    n_indices = tuple((n_indices[i] + (curr_n_indices[i],) for i in range(len(n_indices))))
                    step_beam_scores = tuple((step_beam_scores[i] + (torch.exp(beam_scores[i]),) for i in range(len(step_beam_scores))))
                    step_beams_tokens = tuple((step_beams_tokens[i] + (beam_next_tokens[i],) for i in range(len(step_beams_tokens))))
                    step_parent_indices = tuple((step_parent_indices[i] + (beam_idx[i],) for i in range(len(step_parent_indices))))
                    step_extra_tokens=tuple((step_extra_tokens[i] + (extra_tokens[i],) for i in range(len(step_extra_tokens))))
                    step_extra_scores=tuple((step_extra_scores[i] + (extra_token_scores[i],) for i in range(len(step_extra_scores))))
                    step_extra_ngram_tokens=tuple((step_extra_ngram_tokens[i] + (cur_ngrams[i],) for i in range(len(step_extra_ngram_tokens))))
                    step_extra_ngram_scores=tuple((step_extra_ngram_scores[i] + (cur_ngrams_scores[i],) for i in range(len(step_extra_ngram_scores))))
        ...
        if return_dict_in_generate:
            ...
            if self.config.is_encoder_decoder:
                ...
                return GenerateBeamDecoderOnlyOutput(
                    ...
                    past_key_values=model_kwargs.get("past_key_values"),
                    n_indices=n_indices,
                    step_beam_scores=step_beam_scores,
                    step_beam_tokens=step_beams_tokens,
                    step_parent_indices=step_parent_indices,
                    step_extra_tokens=step_extra_tokens,
                    step_extra_scores=step_extra_scores,
                    step_extra_ngram_tokens=step_extra_ngram_tokens,
                    step_extra_ngram_scores=step_extra_ngram_scores
                )
        else:
